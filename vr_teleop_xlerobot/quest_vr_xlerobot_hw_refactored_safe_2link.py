#!/usr/bin/env python3
"""
Quest VR teleoperation for XLeRobot hardware - REFACTORED VERSION
하드웨어 직접 제어에 최적화 (MuJoCo 의존성 제거)
"""
...
class QuestVRXLeRobotController:
    ...
    def __init__(self, quest_monitor: QuestSocketMonitor, robot: XLerobot):
        self.quest_monitor = quest_monitor
        self.robot = robot
        self.kinematics = SO101Kinematics()

        # ===== 로봇 상태 (degree로 직접 관리) - 공식 zero position =====
        self.arm_state = {
            # Right arm
        ...
        self.base_linear_scale = 0.2
        self.base_angular_scale = 40.0  # 조이스틱 x축 → 회전 속도
        self.base_linear_deadzone = 0.08
        self.base_angular_deadzone = 0.02
    ...
    def process_quest_data(self):
        ...
        self._process_left_arm(quest_data.get("left"), current_time)
        self._process_right_arm(quest_data.get("right"), current_time)
        self._process_base(quest_data.get("agv"))
    ...
    def _process_left_arm(self, left_data, current_time):
        ...
        if self.prev_left_pos is not None:
            ...
            delta_x, delta_y = self._map_vr_delta(delta)
    ...
    def _process_base(self, agv_data):
        if not agv_data:
            self.base_vel = {"x.vel": 0.0, "y.vel": 0.0, "theta.vel": 0.0}
            return

        raw_x = float(np.clip(agv_data.get("x", 0.0), -1.0, 1.0))
        raw_y = float(np.clip(agv_data.get("y", 0.0), -1.0, 1.0))

        y_stick = raw_y if abs(raw_y) >= self.base_linear_deadzone else 0.0

        if abs(raw_y) < self.base_linear_deadzone and abs(raw_x) >= 0.5:
            # Stick pushed mostly sideways: treat as rotation-only
            x_stick = raw_x
        elif abs(raw_x) >= self.base_angular_deadzone and abs(raw_y) >= self.base_linear_deadzone:
            # Diagonal input: project onto dominant axis
            if abs(raw_y) >= abs(raw_x):
                x_stick = 0.0
            else:
                x_stick = raw_x
                y_stick = 0.0
        elif abs(raw_x) >= self.base_angular_deadzone:
            x_stick = raw_x
        else:
            x_stick = 0.0

        self.base_vel["x.vel"] = y_stick * self.base_linear_scale
        self.base_vel["y.vel"] = 0.0
        self.base_vel["theta.vel"] = -x_stick * self.base_angular_scale
    ...
    def _process_head(self, head_data):
        ...
        if self._head_debug_counter % 50 == 0:
            obs = self.robot.get_observation()
            actual_motor1 = obs.get("head_motor_1.pos", 0.0)
            actual_motor2 = obs.get("head_motor_2.pos", 0.0)
            print(f"[HEAD] yaw={yaw:.1f}° pitch={pitch:.1f}° | delta=({delta_yaw:.1f}°, {delta_pitch:.1f}°)")
            print(f"       CMD: motor1={motor1_pos:.1f}° motor2={motor2_pos:.1f}°")
            print(f"       ACT: motor1={actual_motor1:.1f}° motor2={actual_motor2:.1f}°")
    ...
    def _map_vr_delta(self, delta_vec: np.ndarray) -> tuple[float, float]:
        """
    ...
    def initialize_smooth(self, loop_hz=50):
        ...
        for frame in range(init_frames):
            ...
            action = {**self.arm_state, **self.base_vel}
